...
namespace org.denyakicreate.blockcity
{
    public class Main : MonoBehaviour
    {
        ...
        void Update()
        {
            PlayerPrefs.SetString("time-totalplaytime", (TimeManager.totalplaytime += (long)(Time.deltaTime * 1000)).ToString());
            ...
            else if (Input.GetKeyDown(KeyCode.F2))
            {
                ...
                Application.CaptureScreenshot(Path.Combine(ssdirpath, System.DateTime.Now.Ticks.ToString()) + ".png");
            }
            ...
            switch (window)
            {
                ...
                case 1:
                    ...
                    if (statusinfo)
                    {
                        ...
                        GUIManager.label("総計起動時間: " + TimeManager.convinterval(new TimeSpan(TimeManager.totalplaytime * 10000)), GUIManager.Position.LeftBottom);
                    }
                    if (firststart)
                    {
                    ...
                    {
                        GUIManager.label("tips: " + GUIManager.gettips(), GUIManager.Position.TitleBar);
                    }
                ...
                case 2:
                    ...
                    MenuItem[] _2menu = new MenuItem[] { _2audiovolume, new MenuItem(new MenuItem[] { new MenuButton("キャンセル"), new MenuButton("音声再生"), new MenuButton("決定") }), new MenuButton("スクリーンショットフォルダを開く") };
                ...
                case 19:
                    ...
                    GUIManager.menu_center(new string[] { GUIManager.gettips() });
                ...
                case 30:
                    ...
                    MenuItem[] _30menu = new MenuItem[] { new MenuItem(new MenuItem[] { new MenuButton("<color=" + (playercamera.tool == PlayerCamera.Tool.road ? "yellow" : "white") + ">道路</color>"), new MenuButton("<color=" + (playercamera.tool == PlayerCamera.Tool.destroy ? "yellow" : "white") + ">撤去</color>") }) };
                    GUIManager.menu_bottom(_30menu);
                    /*if (_30menu[0].initems[0].clicked)
                    {
                        playSound(ac_click);
                        if (playercamera.tool == PlayerCamera.Tool.road)
                        {
                            playercamera.tool = PlayerCamera.Tool.none;
                        }
                        else
                        {
                            playercamera.tool = PlayerCamera.Tool.road;
                        }
                    }
                    else */if (_30menu[0].initems[1].clicked)
                    {
                        playSound(ac_click);
                        if (playercamera.tool == PlayerCamera.Tool.destroy)
                        {
                            playercamera.tool = PlayerCamera.Tool.none;
                        }
                        else
                        {
                            playercamera.tool = PlayerCamera.Tool.destroy;
                        }
                    }
                ...
                case 31:
                    GUIManager.dark();
                    GUIManager.titlebardark();
                    GUIManager.label("tips: " + GUIManager.gettips(), GUIManager.Position.LeftBottom);
